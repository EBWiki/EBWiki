...
Once you submit your PR for review, GitHub Actions runs our `CI` workflow: RuboCop, RSpec, Brakeman, and markdown-link checks. CodeQL runs as a separate required security scan. GitHub Copilot is the one review bot and may leave first-pass comments; those comments are not an Approve. Required Approve is a human reviewer from `.github/CODEOWNERS` (currently `@gktreviewer`). Do not request CodeRabbit or Factory Droid. Do not request Copilot as an Approver. Public titles and descriptions should read as project copy, not chat. If everything passes, you will see a green check. If something fails, use the Details link, fix what you broke, and note leftover failures that are unrelated to your change.
...
* [Agent evaluation with Harbor](SPIKE_HARBOR.md) (optional; does not replace Docker or RSpec)
