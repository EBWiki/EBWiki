...
RSpec.describe Statistics do
  ...
  def metric_growth(start_metric:, end_metric:)
    Statistics.metric_growth(start_metric: start_metric, end_metric: end_metric)
  end

  def unique_followers_growth(start_metric:, end_metric:)
    Statistics.unique_followers_growth(start_metric: start_metric, end_metric: end_metric)
  end
  ...
  describe 'unique follower growth rate' do
    it 'returns the correct percentage' do
      ...
      expect(unique_followers_growth(start_metric: Follow.occurring_by(30.days.ago),
