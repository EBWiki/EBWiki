...
RSpec.describe AgenciesHelper, type: :helper do
  ...
  describe '#link_to_name' do
    ...
    it 'links by record id' do
      expect(helper.link_to_name(agency)).to include("/agencies/#{agency.id}")
    end
  end

  describe '#link_to_edit' do
    it 'displays formatted edit link' do
      expect(helper.link_to_edit(agency)).to include("/agencies/#{agency.id}/edit")
    end
  end
end
